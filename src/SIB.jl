...
# alg_compatible(prob::LLGProblem,alg::SIBOrdinary) = true
# alg_compatible(prob::LLGProblem,alg::SIBOrdinary) = true
...
function alg_cache(alg::SIBOrdinary, u, rate_prototype, uEltypeNoUnits, uBottomEltypeNoUnits, tTypeNoUnits, uprev, uprev2, f, t, dt, reltol_internal, p, calck, iip)
    Acache = similar(u)
    up = similar(u)
    N = size(u)[1]
...
function alg_cache(alg::SIBOrdinary, prob::StochasticLLGProblem, u, dW, dZ, p, rate_prototype, noise_rate_prototype, jump_prototype, uEltypeNoUnits, uBottomEltypeNoUnits, tTypeNoUnits, uprev, f, t, dt, iip)
    Acache = similar(u)
    up = similar(u)
    N = size(u)[1]
    detM = zeros(Float64, N)
